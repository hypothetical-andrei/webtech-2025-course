...
const BookEditor = (props) => {
  ...
  const saveBook = () => {
    if (isNewBook) {
      dispatch(bookActions.addBook({ title, content }, pageSize, pageNumber, filterField, filterValue, sortField, sortOrder))
    } else {
      dispatch(bookActions.updateBook(selectedBook, { title, content }, pageSize, pageNumber, filterField, filterValue, sortField, sortOrder))
    }
    setIsAddDialogShown(false)
    setTitle('')
    setContent('')
  ...
  const deleteBook = (rowData) => {
    dispatch(bookActions.deleteBook(rowData.id, pageSize, pageNumber, filterField, filterValue, sortField, sortOrder))
  }
  ...
  const opsTemplate = (rowData) => {
    return (
      <>
        ...
        <Button icon='pi pi-arrow-right' className='p-button-success' onClick={() => navigateToBook(rowData)} />
      </>
    )
  }

  const navigateToBook = (rowData) => {
    console.warn('navigating to ', rowData.id)
    navigate(`/books/${rowData.id}`)
  }
  ...
  console.warn('rendering BookEditor', { pageNumber, pageSize, filterField, filterValue, sortField, sortOrder } )
  return (
    <>
      <DataTable 
        value={bookList} 
